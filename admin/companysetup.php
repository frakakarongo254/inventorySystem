		...
		if(isset($_GET['success'])){
		 ...
		 if(isset($_POST['companyProfile'])){
			...
			echo $vatcharges=$_POST['vatcharges'];
			 $coderesult= mysqli_query($conn,"select * from `company_details` where `id` !='' ");
			 if ($coderesult->num_rows > 0) {
				 
				$result=mysqli_query($conn,"update `company_details` SET name= '".$companyName."',vat= '".$vatcharges."',address= '".$address."',phone= '".$phone."',email= '".$email."'  ");
							if($result){
						...
						$result=mysqli_query($conn,"insert into `company_details` (name,vat,address,email,phone) values('$companyName','$vatcharges','$address','$email','$phone')");
					if($result){
